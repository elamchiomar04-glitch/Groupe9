...
def _(mo):
    ...
    - L'axe $x$ représente la **verticale** et est dirigé vers le haut.
    - L'axe $y$ représente l'**horizontale** et est dirigé vers la gauche.

    Dans cette configuration, l'angle 0 correspond à un alignement parfait avec l'axe vertical $x$. L'angle total de la force de poussée par rapport à cet axe vertical est la somme de l'inclinaison du propulseur ($\theta$) et de l'orientation de la tuyère ($\phi$).

    En utilisant la trigonométrie classique, on peut donc décomposer la force $f$ du réacteur sur nos deux axes :
    - Composante verticale : $f_x = f \cos(\theta + \phi)$
    - Composante horizontale : $f_y = f \sin(\theta + \phi)$
    """)
    return
...
def _(mo):
    ...
    - $\vec{P} = -Mg \vec{x}$
    - $\vec{F}_{R} = f_x \vec{x} + f_y \vec{y}$

    On obtient le système d'équations différentielles suivant pour la position $(x, y)$ :

    $$M \ddot{x} = f_x - M g \implies \ddot{x} = \frac{f_x}{M} - g$$
    $$M \ddot{y} = f_y \implies \ddot{y} = \frac{f_y}{M}$$
    """)
    return
...
def _(J, M, g, l, np):
    def F(s, f, phi):
        ...
        dvx = (f * np.cos(theta + phi) / M) - g
        dvy = (f * np.sin(theta + phi) / M)
